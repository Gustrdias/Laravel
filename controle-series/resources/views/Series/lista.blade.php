<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title>Series</title>
		<link rel="stylesheet"
			href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T"
			crossorigin="anonymous">
    </head>
    <body>
	
<div class="container">
	<div class="row">
	<div class="col-sm-2">
	<a href="/estabelecimentos/adicionar" class="btn btn-dark mb-2">Criar Novo</a>
<ul class="list-group">
@foreach($data as $estab)
<li class="list-group-item"> {{$estab['nomeEstab']}} - {{$estab['plataforma']}}</li>
@endforeach
</ul>
	</div>
	<div class="col-sm-2">
  <h2>Lista de Séries</h2>          
  <table class="table table-bordered">
    <thead>
      <tr>
        <th>Nome</th>
        <th>Genero</th>
		<th>Origem</th>
		<th>Ano</th>
		<th>Duração</th>
		<th>Temporada</th>
		<th>Episódios</th>
		<th>Desc.Idioma</th>
		<th>Comentário</th>
		<th>Avaliação</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td></td>
      </tr>
      <tr>
        <td></td>
      </tr>
	   <tr>
        <td></td>
      </tr>
	   <tr>
        <td></td>
      </tr>
	   <tr>
        <td></td>
      </tr>
	   <tr>
        <td></td>
      </tr>
	   <tr>
        <td></td>
      </tr>
	   <tr>
        <td></td>
      </tr>
	   <tr>
        <td></td>
      </tr>
	   <tr>
        <td></td>
      </tr>
    </tbody>
  </table>
  </div>
  <div class="col-sm-2">
	</div>
  </div>
</div>
    </body>
</html>
